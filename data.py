...
class TextureDataset(Dataset):
    ...
    def __init__(self, root: Path, split_file: Path, passive_context: int = 4, crop_size: int | None = None, random_crop: bool = False) -> None:
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor | str | int]:
        ...
        texture1 = _normalise(_load_frame(self.root, scene, 'texture', left + 10))
        ...
        flow_path = _frame_path(self.root, scene, 'flow', target_id)
        if not flow_path.is_file():
            raise FileNotFoundError(f'MultipleFlowLoss requires a 4-channel flow label: {flow_path}')
        flow = np.load(flow_path).astype(np.float32)
        if flow.ndim != 3 or flow.shape[0] != 4:
            raise ValueError(f'Expected flow shape [4, H, W] at {flow_path}, got {flow.shape}')
