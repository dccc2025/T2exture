"""AMT-L fine-tuning model with optional passive-context conditioning."""
...
def _load_amt_l() -> type[nn.Module]:
    """Load the vendored AMT-L class without changing its original source layout."""
    package_root = Path(__file__).parents[1] / 'third_party' / 'AMT_official'
    path = package_root / 'networks' / 'AMT-L.py'
    spec = importlib.util.spec_from_file_location('t2texture_amt_l', path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f'Cannot load AMT-L from {path}')
...
class TextureAdapter(nn.Module):
    """Adapt one-channel texture frames to AMT-L's RGB input space with time conditioning."""

    def __init__(self, condition_dim: int = 64) -> None:
        """Initialize a replication-style 1-to-3 channel adapter and time bias projection."""
        super().__init__()
    ...
    def forward(self, image: torch.Tensor, condition: torch.Tensor) -> torch.Tensor:
        """Convert a texture image to RGB and add a learned timestamp-dependent bias."""
        if image.shape[1] != 1:
...
class PassivePyramidEncoder(nn.Module):
    """Encode the two-before/two-after passive context at AMT decoder resolutions."""

    def __init__(self, context_size: int, condition_dim: int = 64) -> None:
        """Build a three-level passive pyramid and one time bias per pyramid level."""
        super().__init__()
        self.context_size = context_size
        self.level1 = nn.Sequential(nn.Conv2d(context_size, 48, 3, 2, 1), nn.PReLU(48), nn.Conv2d(48, 48, 3, 1, 1), nn.PReLU(48))
        ...
        self.time_biases = nn.ModuleList([nn.Linear(condition_dim, channels) for channels in (48, 64, 72)])
    ...
    def forward(self, passive_context: torch.Tensor, condition: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """Return passive features aligned with AMT-L decoder levels one through three."""
...
class T2textureAMTL(nn.Module):
    """Fine-tune AMT-L using texture endpoints and optional passive observations."""

    def __init__(self, pretrained: str | Path, passive_context: int = 4) -> None:
        """Load AMT-L and attach time-conditioned passive zero-conv adapters."""
        super().__init__()
        ...
        self.passive_encoder = PassivePyramidEncoder(passive_context, condition_dim=64) if passive_context else None
        self.zero_convs = nn.ModuleList([nn.Conv2d(channels, channels, 1) for channels in (48, 64, 72)]) if passive_context else nn.ModuleList()
        for layer in self.zero_convs:
            nn.init.zeros_(layer.weight)
            nn.init.zeros_(layer.bias)
        self.backbone = _load_amt_l()()
    ...
    def _inject(self, output: tuple[torch.Tensor, torch.Tensor, torch.Tensor], index: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """Add the zero-conv passive residual to a decoder's feature output."""
    ...
    def _inject_level3(self, _module: nn.Module, _inputs: tuple[torch.Tensor, ...], output: tuple[torch.Tensor, torch.Tensor, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """Inject passive features into AMT-L decoder stage four."""
    ...
    def _inject_level2(self, _module: nn.Module, _inputs: tuple[torch.Tensor, ...], output: tuple[torch.Tensor, torch.Tensor, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """Inject passive features into AMT-L decoder stage three."""
    ...
    def _inject_level1(self, _module: nn.Module, _inputs: tuple[torch.Tensor, ...], output: tuple[torch.Tensor, torch.Tensor, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """Inject passive features into AMT-L decoder stage two."""
        return self._inject(output, 0)

    def set_phase(self, phase: str) -> None:
        """Freeze AMT-L or unfreeze only the rear refinement modules."""
    ...
    def forward(self, texture0: torch.Tensor, texture1: torch.Tensor, time: torch.Tensor, passive_context: torch.Tensor, return_flow: bool = False) -> dict[str, torch.Tensor | list[torch.Tensor]]:
        """Predict one texture frame and optionally expose AMT-L multi-scale flows for training."""
