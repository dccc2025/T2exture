"""AMT-L fine-tuning model with four-frame passive-context conditioning."""
...
def _load_amt_l() -> type[nn.Module]:
    ...
    path = Path(__file__).parents[1] / 'third_party' / 'amt' / 'AMT-L.py'
...
class T2textureAMTL(nn.Module):
    """Fine-tune AMT-L using texture endpoints and four adjacent passive observations."""
    ...
    def __init__(self, pretrained: str | Path, passive_context: int = 4) -> None:
        ...
        self.passive_encoder = PassivePyramidEncoder(passive_context, condition_dim=64)
        self.zero_convs = nn.ModuleList([nn.Conv2d(channels, channels, 1) for channels in (48, 64, 72)])
    ...
    def set_phase(self, phase: str) -> None:
        """Freeze AMT-L for adapter training or unfreeze it for whole-model fine-tuning."""
