"""AMT-S T2exture wrapper with optional passive-context conditioning."""
...
class T2textureAMTS(nn.Module):
    """Fine-tune AMT-S using texture endpoints and optional passive observations."""
    ...
    def __init__(self, pretrained: str | Path, passive_context: int = 4) -> None:
        ...
        self.passive_encoder = PassivePyramidEncoderS(passive_context, condition_dim=64) if passive_context else None
        self.zero_convs = nn.ModuleList([nn.Conv2d(channels, channels, 1) for channels in (20, 32, 44)]) if passive_context else nn.ModuleList()
