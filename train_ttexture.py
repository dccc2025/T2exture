...
def arguments():
    ...
    p.add_argument('--data-root', type=Path, default=Path('/essfs10/daicheng/TT_sim'))
...
def flatten_batch(batch: dict, device: torch.device):
    ...
    return x0, x1, p0, p1, passive, target, times, b, n


def losses_for(model, batch, device, train: bool):
    x0, x1, p0, p1, passive, target, times, b, n = flatten_batch(batch, device)
    with autocast(enabled=device.type == 'cuda'):
        prediction = model(x0, x1, times, p0, p1, passive).mean(1, keepdim=True)
        ...
        total = rec + 0.15 * css + 0.20 * edge + 0.10 * temp
...
def validate(model, loader, device):
    ...
    for batch in loader:
        _, values = losses_for(model, batch, device, False)
...
def main():
    ...
    train_set = TextureSequenceDataset(args.data_root, split_dir / 'train.txt', crop_size=args.crop)
    valid_set = TextureSequenceDataset(args.data_root, split_dir / 'valid.txt', crop_size=None)
    ...
    for phase, phase_epochs, learning_rate in phases:
        ...
        for _ in range(phase_epochs):
            ...
            for batch in train_loader:
                ...
                total, values = losses_for(model, batch, device, True)
            ...
            valid_values = validate(model, valid_loader, device)
