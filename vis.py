...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--error-cmap', choices=['gray', 'error'], default='gray')
...
def _write_video(path: Path, frames: list[Path], fps: int) -> None:
    ...
    if ffmpeg is None:
        raise RuntimeError('ffmpeg is required to write MP4 videos but was not found on PATH')
    with tempfile.TemporaryDirectory(prefix='t2exture_vis_') as tmp:
        tmp_dir = Path(tmp)
        for index, frame_path in enumerate(frames):
            ...
            if width % 2 or height % 2:
                ...
                frame = padded
            frame.save(tmp_dir / f'frame_{index:06d}.png')
        command = [
            ffmpeg,
            '-y',
            '-loglevel',
