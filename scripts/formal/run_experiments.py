...
``configs/formal/experiment_manifest.yaml``. It does not change experiment
...
TABLE07_CONFIG = Path('configs/formal/table07/passive-guidance-no-temporal.yaml')
...
def sync_metrics() -> None:
    run_command(
        [
            PYTHON,
            '-B',
            'scripts/formal/collect_metrics.py',
            '--outputs-root',
            str(OUTPUTS_ROOT),
            '--csv',
...
def train_eval_vis(exp_dir: Path, config: Path, method_label: str, log_prefix: str, train_script: Path = Path('train.py')) -> None:
...
def run_table05() -> None:
    ...
    for backbone, setting, row_id in TABLE05_ROWS:
        ...
        command = [
            PYTHON,
            '-B',
            'scripts/formal/profile_deployment.py',
            '--backbone',
            backbone,
            '--setting',
            setting,
    ...
    for method in TABLE06_METHODS:
        ...
        run_command(
            [
                PYTHON,
                '-B',
                'scripts/real/evaluate_benchmark.py',
                '--method',
                method,
                '--config',
                str(TABLE06_CONFIG),
...
def run_table07() -> None:
    """Run only the Ours-L middle rows for the component ablation table."""
    train_eval_vis(
        OUTPUTS_ROOT / 'table03_passive_context' / 'context-per-side-00',
        Path('configs/formal/table03/context-per-side-00.yaml'),
        'T2V',
        'table07_t2v_adapter',
    )
    train_eval_vis(
        OUTPUTS_ROOT / 'table07_component_ablation' / 'passive-guidance-no-temporal',
        TABLE07_CONFIG,
        'Passive-Guidance-No-Temporal',
        'table07_passive_guidance_no_temporal',
        train_script=Path('scripts/formal/train_ours_l_no_passive_temporal.py'),
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--table07', action='store_true', help='Run missing Ours-L component-ablation middle rows.')
    parser.add_argument('--table03-start', type=int, default=0)
...
def main() -> None:
    args = parse_args()
    run_any = args.table03 or args.table04 or args.table05 or args.table06 or args.table07
    if not run_any:
        args.table03 = True
        args.table04 = True
        args.table05 = True
