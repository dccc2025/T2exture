"""Evaluate official IFRNet checkpoints on the T2exture dataset split."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--checkpoint', type=Path, default=Path('pretrained/IFRNet_Vimeo90K.pth'))
...
def load_ifrnet_class() -> type[nn.Module]:
    package_root = ROOT / 'third_party' / 'IFRNet'
    path = package_root / 'models' / 'IFRNet.py'
    spec = importlib.util.spec_from_file_location('t2texture_ifrnet_model', path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f'Cannot load IFRNet from {path}')
...
def load_model(checkpoint_path: Path, device: torch.device) -> nn.Module:
    model = load_ifrnet_class()()
    checkpoint = _torch_load(checkpoint_path, device)
    result = model.load_state_dict(_state_dict(checkpoint), strict=False)
    if result.missing_keys or result.unexpected_keys:
        raise RuntimeError(
            'IFRNet checkpoint does not match official IFRNet.Model. '
            f'missing={len(result.missing_keys)}, unexpected={len(result.unexpected_keys)}. '
            'Use the official IFRNet_Vimeo90K.pth VFI checkpoint, not LiteFlowNet.'
        )
