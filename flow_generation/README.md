...
```powershell
conda run -n t2exture python -B -m flow_generation.generate_liteflownet_flow --data-root datasets --active-stride 10 --splits train valid test --device cuda
```

Generate another active-sparsity set:

```powershell
conda run -n t2exture python -B -m flow_generation.generate_liteflownet_flow --data-root datasets --active-stride 5 --splits train valid test --device cuda
```
